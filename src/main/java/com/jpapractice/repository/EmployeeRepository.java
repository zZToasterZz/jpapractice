...
{
	@Query(value = "select a from Employee a where a.employee_id like CONCAT('%',:id,'%') and a.name like CONCAT('%',:name,'%') and a.address like CONCAT('%',:address,'%') and a.city like CONCAT('%',:city,'%')")
